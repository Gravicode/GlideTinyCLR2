...
namespace System.Xml
{
  internal class Res
  {
    private static string[] Xml_Errors = new string[61]
    {
      "{0}",
    ...
    {
      return Res.Format(format, 0, arg0);
    }
    ...
    private static string Format(string format, object[] args)
    {
      ...
      for (int argIndex = 0; argIndex < args.Length; ++argIndex)
        format1 = Res.Format(format1, argIndex, args[argIndex]);
      return format1;
    }

    private static string Format(string format, int argIndex, object argValue)
    {
      string str1 = "{" + (object) argIndex + "}";
      int length = format.IndexOf(str1);
      if (length == -1)
        return format;
      string str2 = format.Substring(0, length);
      return (argValue != null ? str2 + argValue.ToString() : str2 + "null") + format.Substring(length + str1.Length);
    }
  }
}
